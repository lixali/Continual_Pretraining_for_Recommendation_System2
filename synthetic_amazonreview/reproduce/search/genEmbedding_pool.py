...
def genEmbedding(model, pool_documents_dataloader, device, logging):
    ...
    with torch.no_grad():
        ...
        if isinstance(pool_document_key_list[0], np.int64): pool_document_key_list = pool_document_key_list.tolist()
...
def main(args):
    ...
    logger.info(f"############ Finish generating the embedding pickle files ###########")
    end_time = time.time()
