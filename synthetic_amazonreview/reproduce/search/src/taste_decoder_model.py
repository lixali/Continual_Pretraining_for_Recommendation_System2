...
class DRD4RecModel(nn.Module):
    ...
    def forward(
        ...
        # print(f"q_reps.shape and p_reps.shape is {q_reps.shape} {p_reps.shape}")
        scores = torch.matmul(q_reps, p_reps.transpose(0, 1))

        target = torch.arange(scores.size(0), device=scores.device, dtype=torch.long)
        target = target * self.data_args.train_n_passages
        # print(f"target.shape and score.shape is {target.shape} {scores.shape} ")
    ...
    def encode(self, items, model, head):
        ...
        result = model(input_ids, attention_mask)  # hidden states, 1st token embedding
        # unwrap the last hidden state and the first token embedding here
        hidden = result.last_hidden_state
        reps = hidden[torch.arange(hidden.size(0)), sequence_lengths]  # bz * d
        # reps = hidden[:, sequence_lengths, :] # bz
        if head is not None:
            reps = head(reps)  # D * d
        if self.normalize:
    ...
    def build(
        ...
        **hf_kwargs,
        ...
        base_model = model_class.from_pretrained(model_name_or_path, **hf_kwargs)
        # a pass on these encoders should go to the AutoModel, not the custom model -> no halving passages
        lm_q = AutoModel.from_config(base_model.config)
