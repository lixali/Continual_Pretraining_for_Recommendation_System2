...
class TasteCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        for q_collated in q_collated_list:
            ...
            cur_item = item_input_ids.size(
                0
            )  # number of parts -> 1 for decoder-only case
            if not isinstance(self.tokenizer, PreTrainedTokenizerFast):
                # the encoder attention sparsity modification
                ...
                seq_input_ids.append(
                    item_input_ids
                )  # n_passage * dim -> gpt input: bsz*seq_len
...
class StreamDRTrainDataset(StreamTrainDatasetMixin, TasteTrainDataset):
    print("using the stream dataset...")
    pass
