...
def get_args():
    ...
    parser.add_argument("--num_passage", type=int, default=2)
    parser.add_argument("--text_alike", type=str)
...
def main():
    ...
    #### added by Lixiang accomadate the change to synthetic data generation
    with open(args.text_alike, "r") as f:
        pos_list = []
        for line in f:

            data = json.loads(line)
            pos_list.append(data["2_id"])
             
    for i,pos in enumerate(pos_list):
        train_data_ids[i][-1] = pos 
        train_data[i][-1] = pos
    
    #### endding added by Lixiang
